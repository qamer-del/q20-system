...
export default function Sidebar({ mobile, role = "VIEWER" }: { mobile?: boolean; role?: string } = {}) {
  ...
  const links = allLinks.filter(link => {
    ...
    if (role === "ACCOUNTANT") return ["/dashboard", "/inventory", "/accounting", "/invoices", "/reporting", "/purchases"].includes(link.href);
    ...
    <aside className={`w-64 bg-slate-900 border-r border-slate-800 text-white min-h-screen flex-col flex-shrink-0 sticky top-0 h-screen overflow-y-auto ${mobile ? "flex" : "hidden lg:flex"}`}>
      <div className="p-6 flex items-center gap-4 border-b border-slate-800 backdrop-blur-md bg-slate-900/50 sticky top-0 z-10">
        ...
        <div>
           ...
           <span className="block text-[8px] text-indigo-400 font-bold uppercase tracking-widest mt-1">Management Engine</span>
        </div>
      </div>
