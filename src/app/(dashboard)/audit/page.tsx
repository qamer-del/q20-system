                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-sm font-medium">
                  {logs.map((log: any) => (
                    <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                      ...
                      <td className="p-6">
                        <span className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm ${log.action.includes("RECONCILE") ? 'bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-900' :
                  ...
                  {logs.length === 0 && (
                    <tr>
                      <td colSpan={4} className="p-16 text-center">
                        ...
                        <p className="text-slate-500 font-bold text-lg">{(dict.Audit as any).system_secure}</p>
                        <p className="text-slate-400 text-sm mt-2">{(dict.Audit as any).audit_empty_desc}</p>
                      </td>
                    </tr>
                  )}
                </tbody>
