      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3">
            <ActionForm action={processRefillPurchase} successMessage="Fuel delivery registered successfully!" className="h-full">
              <Card className="border-t-4 border-t-blue-600 shadow-xl h-full flex flex-col">
                ...
                <CardContent className="flex-1 space-y-6">
                  <div className="space-y-3">
                    ...
                    <select name="supplierId" required className="flex h-12 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 py-2 text-sm font-medium transition-all focus-visible:outline-none focus:ring-2 focus:ring-blue-500">
                      <option value="">{(dict.Purchases as any).select_vendor}</option>
                  ...
                  <div className="space-y-3">
                    ...
                    <select name="tankId" required className="flex h-12 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 py-2 text-sm font-medium transition-all focus-visible:outline-none focus:ring-2 focus:ring-blue-500">
                      <option value="">{(dict.Purchases as any).select_destination}</option>
                      {tanks.map((t: any) => (
                  ...
                  <div className="space-y-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{(dict.POS as any).payment_method}</label>
                    <div className="flex gap-4">
                      <label className="flex-1 cursor-pointer">
                        <input type="radio" name="paymentMethod" value="CASH" className="peer hidden" required />
                        <div className="peer-checked:bg-emerald-600 peer-checked:text-white peer-checked:border-emerald-600 peer-checked:shadow-lg bg-slate-50 dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-center font-bold transition-all uppercase tracking-widest text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">{(dict.POS as any).cash}</div>
                      </label>
                      <label className="flex-1 cursor-pointer">
                        <input type="radio" name="paymentMethod" value="BANK" className="peer hidden" />
                        <div className="peer-checked:bg-indigo-600 peer-checked:text-white peer-checked:border-indigo-600 peer-checked:shadow-lg bg-slate-50 dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-center font-bold transition-all uppercase tracking-widest text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">Bank</div>
                      </label>
                    </div>
                  </div>

                  <SubmitButton variant="default" className="w-full h-16 text-lg mt-4 shadow-xl">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="shadow-xl">
              <CardHeader className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                <CardTitle className="text-lg flex items-center gap-2"><Users className="text-emerald-500 w-5 h-5" /> {(dict.Purchases as any).active_vendors}</CardTitle>
                <CardDescription>{(dict.Purchases as any).vendor_desc}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <ActionForm action={addSupplier} successMessage="Vendor added successfully!" className="space-y-4">
                  ...
                  <SubmitButton variant="secondary" className="w-full text-xs uppercase tracking-widest"><Plus className="w-4 h-4 mr-2" /> {(dict.Purchases as any).add_vendor}</SubmitButton>
                </ActionForm>

                {suppliers.length > 0 && (
                  <div className="space-y-2 border-t border-slate-100 dark:border-slate-800 pt-6">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3">{(dict.Purchases as any).vendor_directory}</h3>
                ...
                <div className="space-y-3">
                  {recentPurchases.length === 0 && (
                    <div className="p-8 text-center text-slate-400 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl">
                      <span className="italic block mb-2">{(dict.Purchases as any).no_deliveries}</span>
                      <span className="text-[10px] uppercase font-bold tracking-widest">{(dict.Purchases as any).awaiting_shipment}</span>
                    </div>
                  )}
